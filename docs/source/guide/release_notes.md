...
This section highlights the new features and enhancements, and bug fixes in Label Studio Enterprise 2.3.1.

### New features and enhancements 
Label Studio Enterprise 2.3.1 introduces the following new features and enhancements.

- Allows annotators and reviewers to filter view of transcriptions by author name [DEV-2669].
...
- Implement new columns for commentary system: comment count, unresolved comment count, comment authors, last comment date [DEV-2885].
- Introduce size presets to zoom an image to fit with in the viewport or to have it at its natural size (up to available space in viewport). With this release, you can now set the image to positioned vertically (top, center, bottom) and horizontally (left, center, right) [DEV-2504].
...
- Add the ability to sustain collapse state of the label across tasks and maintain consistency in the order of the label groups [DEV-2755].
...
- Block the entire screen by non-closable modal window only when trial ends [DEV-399].
...
- Introduce new attributes for `<Audio/>` tag: `defaultZoom`, `defaultSpeed` and `defaultVolume` [DEV-388].
...
- Improve revoke invite UX so its consistent when used from the selected user section and the revoke invite button in the dropdown in User list [DEV-3196].
...
- Fixed "Tasks per page" field that should be in sync with number of tasks displayed [DEV-2170].
- Prevented crash when a user opens a task by direct link [DEV-2526].
...
- Added a change in rotate anchor that is no longer out of the bbox and now are close to all resize anchors [DEV-2671].
...
- `get_local_path` doesn't work for local-files in ML backends and converter [DEV-2827].
...
- Resolved an error where 3 point bbox would remain usable after removing rectangles from the labeling configuration [DEV-2696].
...
- Implemented new columns for commentary system: comment count, unresolved comment count, comment authors, last comment date [DEV-2885].
- Locked polygons doesn't show the editable points any longer [DEV-2977].
...
- Fixed an issue when panels can fall out of the viewport if viewport's size changed [DEV-2943].
...
- Unfinished polygon region was not auto-completed when user moved it [DEV-2514].
...
- Fixed an issue where missing empty body was generated for 204 responses [DEV-3014].
- Broken "All Projects" pagination [DEV-3125].
- Fixed an issue with paragraph regions was not selectable within the new Outliner [DEV-3030].
...
- Improved polygon point removal during drawing: you can use usual undohotkeys (ctrl/cmd+z) to remove the point you just set or redo it if you want (ctrl/cmd+shift+z) [DEV-2576].
...
- Fixed an issue when locking UI disappear when "trial days" is negative [DEV-3275].
...
- Deleted tasks was not working with some ordering (e.g. by annotators) [DEV-3313].
...
- Fixed gaps on image border on different screen sizes which may lead to slight region subpixel shifts [DEV-3322].
...
- Fixed Completed field in case maximum annotations changes after overlap change [DEV-3387].
- Created the possibility to enable pagination in repeater for performance improvement [DEV-3298].
...
- When page was selected from region, item per page was changed to 1 and selected item was displayed [DEV-3428].
...
- Switch to drawing tools while in the process of drawing a new region was not supported [DEV-1943].
...
- Data manager broke when annotator was deactivated [DEV-3520, DEV-3465].
...
- Added `CreateOnlyFieldsMixin` and adding to `BaseUserSerializer` to make email only write-able on create [DEV-3430].
...
- Fixed selecting regions in outline and text when browse history [DEV-3485].
...
- Denied removing user by API [DEV-3598].
...
- Added validation if stage is ready [DEV-3607].
